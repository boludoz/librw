...
#if defined(DEPTHPROBE_UPDATE)
layout(set = 0, binding = 0, rg16f)     uniform image2D uOutDepth;
#else
...
void main()
{
    ...
    const bool inactive = probeStates[relativeProbeId] == DDGI_PROBE_STATE_INACTIVE;
    const bool outOfCam = probeStates[relativeProbeId] == DDGI_PROBE_STATE_OUTOF_CAMERA;

    if(inactive || outOfCam)
        return;
    ...
    result.rgb = pow(result.rgb, vec3(1.0f / ddgi.energyPreservation));
#endif
