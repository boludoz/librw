...
namespace rw
{
	namespace vulkan
	{
		...
		Shader* Shader::createComp(const std::string& comp, const char* userDefine)
		{
			Shader* sh = rwNewT(Shader, 1, MEMDUR_EVENT | ID_DRIVER);        // or global?
			std::vector<uint32_t> computeShader;
			maple::ShaderCompiler::complie(maple::ShaderType::Compute, comp.c_str(), computeShader, userDefine);
			auto shaderPtr = maple::Shader::create(computeShader);
			sh->shaderId = shaders.size();
			shaders.push_back(shaderPtr);
			return sh;
		}
		...
		void Shader::setConstValue(const std::string &name, const void *value)
		{ 
			getShader(shaderId)->getPushConstant(0)->setValue(name, value);
		}
