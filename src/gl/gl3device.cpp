...
const char *shaderDecl120 = "#version 120\n"
                            "#define GL2\n"
...
static uint32 stencilOpMap[] = {GL_KEEP, // actually invalid
                                GL_KEEP, GL_ZERO,   GL_REPLACE,   GL_INCR,
                                GL_DECR, GL_INVERT, GL_INCR_WRAP, GL_DECR_WRAP};

static uint32 stencilFuncMap[] = {GL_NEVER, // actually invalid
...
void flushGlRenderState(void) {
  ...
  if (oldGlState.stencilFunc != curGlState.stencilFunc ||
    ...
    oldGlState.stencilRef = curGlState.stencilRef;
    oldGlState.stencilMask = curGlState.stencilMask;
    glStencilFunc(oldGlState.stencilFunc, oldGlState.stencilRef,
                  oldGlState.stencilMask);
  }
  if (oldGlState.stencilPass != curGlState.stencilPass ||
    ...
    oldGlState.stencilFail = curGlState.stencilFail;
    oldGlState.stencilZFail = curGlState.stencilZFail;
    glStencilOp(oldGlState.stencilFail, oldGlState.stencilZFail,
                oldGlState.stencilPass);
  }
...
static GLint filterConvMap_MIP[] = {0,
                                    GL_NEAREST,
                                    GL_LINEAR,
...
static GLint addressConvMap[] = {0, GL_REPEAT, GL_MIRRORED_REPEAT,
...
static void setFilterMode(uint32 stage, int32 filter, int32 maxAniso = 1) {
  if (rwStateCache.texstage[stage].filter != (Texture::FilterMode)filter) {
    ...
    if (raster) {
      Gl3Raster *natras = PLUGINOFFSET(
          Gl3Raster, rwStateCache.texstage[stage].raster, nativeRasterOffset);
  ...
  case CULLMODE:
    if (rwStateCache.cullmode != value) {
      ...
      if (rwStateCache.cullmode == CULLNONE)
        ...
        setGlRenderState(RWGL_CULLFACE, rwStateCache.cullmode == CULLBACK
                                            ? GL_BACK
                                            : GL_FRONT);
      }
    }
...
int32 setLights(WorldLights *lightData) {
  ...
  for (i = 0; i < lightData->numDirectionals && i < 8; i++) {
    ...
    memcpy(&uniformObject.lightDirection[n], &l->getFrame()->getLTM()->at,
           sizeof(V3d));
    bits |= VSLIGHT_DIRECT;
    n++;
    if (n >= MAX_LIGHTS)
  ...
  for (i = 0; i < lightData->numLocals; i++) {
    ...
    case Light::POINT:
      ...
      uniformObject.lightColor[n] = l->color;
      memcpy(&uniformObject.lightPosition[n], &l->getFrame()->getLTM()->pos,
             sizeof(V3d));
      bits |= VSLIGHT_POINT;
      n++;
      if (n >= MAX_LIGHTS)
...
void flushCache(void) {
  ...
  if (uniformStateDirty[RWGL_FOG] || uniformStateDirty[RWGL_FOGSTART] ||
      uniformStateDirty[RWGL_FOGEND]) {
    float fogData[4] = {uniformState.fogStart, uniformState.fogEnd,
                        uniformState.fogRange, uniformState.fogDisable};
  ...
  if (stateDirty) {
    ...
    uniformState.fogRange =
        1.0f / (rwStateCache.fogStart - rwStateCache.fogEnd);
  ...
  if (zbuf) {
    if (natfb->fboMate == zbuf) {
      ...
      if (natzb->fboMate) {
        ...
        Gl3Raster *oldfb =
            PLUGINOFFSET(Gl3Raster, natzb->fboMate, nativeRasterOffset);
        if (oldfb->fbo) {
...
static void makeVideoModeList(int displayIndex) {
  ...
  for (i = 0; i < glGlobals.numModes; i++) {
    ...
    for (glGlobals.modes[i].depth = 1; glGlobals.modes[i].depth < depth;
         glGlobals.modes[i].depth <<= 1)
      ;
  }
}
...
static int startSDL2(void) {
  ...
  for (i = 0; profiles[i].gl; i++) {
    ...
    if (mode->flags & VIDEOMODEEXCLUSIVE) {
      win = SDL_CreateWindow(
          glGlobals.winTitle, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
          mode->mode.w, mode->mode.h,
      ...
      win = SDL_CreateWindow(
          glGlobals.winTitle, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
          glGlobals.winWidth, glGlobals.winHeight,
          SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
#else
      win = SDL_CreateWindow(glGlobals.winTitle, SDL_WINDOWPOS_UNDEFINED,
                             SDL_WINDOWPOS_UNDEFINED, glGlobals.winWidth,
                             glGlobals.winHeight,
  ...
  if (!((gl3Caps.gles ? gladLoadGLES2Loader : gladLoadGLLoader)(
          (GLADloadproc)SDL_GL_GetProcAddress, gl3Caps.glversion))) {
  ...
  GLint maxVertexAttribs, maxVertexUniforms, maxFragmentUniforms,
      maxTextureSize;
...
static void makeVideoModeList(SDL_DisplayID displayIndex,
  ...
  currentMode = SDL_GetCurrentDisplayMode(displayID);
  // Bug fix: Use displayID not displayIndex for SDL_GetFullscreenDisplayModes
  modes = SDL_GetFullscreenDisplayModes(displayID, &num);

  rwFree(glGlobals.modes);
  // Initialize numModes to 0 first
  glGlobals.numModes = 0;

  // On Android, SDL_GetFullscreenDisplayModes may return NULL or empty
  // In that case, we use the current display mode as fallback
  if (modes == nil || num <= 0) {
    if (currentMode != nil) {
      // Fallback: use current display mode as the only available mode
      glGlobals.modes = rwNewT(DisplayMode, 1, ID_DRIVER | MEMDUR_EVENT);
      glGlobals.modes[0].mode = *currentMode;
      glGlobals.modes[0].flags = 0;
      glGlobals.numModes = 1;
      depth = SDL_BITSPERPIXEL(glGlobals.modes[0].mode.format);
      for (glGlobals.modes[0].depth = 1; glGlobals.modes[0].depth < depth;
           glGlobals.modes[0].depth <<= 1)
        ;
    } else {
      // Last resort: use window size from glGlobals
      glGlobals.modes = rwNewT(DisplayMode, 1, ID_DRIVER | MEMDUR_EVENT);
      memset(&glGlobals.modes[0].mode, 0, sizeof(SDL_DisplayMode));
      glGlobals.modes[0].mode.w = glGlobals.winWidth;
      glGlobals.modes[0].mode.h = glGlobals.winHeight;
      glGlobals.modes[0].mode.format = SDL_PIXELFORMAT_RGBA8888;
      glGlobals.modes[0].mode.refresh_rate = 60.0f;
      glGlobals.modes[0].flags = 0;
  ...
  glGlobals.modes = rwNewT(DisplayMode, num + (currentMode != nil ? 1 : 0),
...
static int startSDL3(void) {
  ...
  for (i = 0; profiles[i].gl; i++) {
    ...
    if (mode->flags & VIDEOMODEEXCLUSIVE) {
      win = SDL_CreateWindow(glGlobals.winTitle, mode->mode.w, mode->mode.h,
                             SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL |
...
#if defined(__ANDROID__) || defined(ANDROID)
      win = SDL_CreateWindow(
          glGlobals.winTitle, glGlobals.winWidth, glGlobals.winHeight,
          SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
#else
      win = SDL_CreateWindow(glGlobals.winTitle, glGlobals.winWidth,
                             glGlobals.winHeight,
                             SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL);
#endif
      if (win)
...
static void makeVideoModeList(GLFWmonitor *monitor) {
  int i, num;
  ...
  for (i = 0; i < glGlobals.numModes; i++) {
    num = glGlobals.modes[i].mode.redBits + glGlobals.modes[i].mode.greenBits +
          glGlobals.modes[i].mode.blueBits;
    // set depth to power of two
    for (glGlobals.modes[i].depth = 1; glGlobals.modes[i].depth < num;
         glGlobals.modes[i].depth <<= 1)
      ;
  }
}
...
static int startGLFW(void) {
  ...
  for (i = 0; profiles[i].gl; i++) {
    ...
    if (mode->flags & VIDEOMODEEXCLUSIVE)
      win = glfwCreateWindow(mode->mode.width, mode->mode.height,
                             glGlobals.winTitle, glGlobals.monitor, nil);
    else
      win = glfwCreateWindow(glGlobals.winWidth, glGlobals.winHeight,
                             glGlobals.winTitle, nil, nil);
    if (win) {
  ...
  /* Init GLAD */
  if (!((gl3Caps.gles ? gladLoadGLES2Loader : gladLoadGLLoader)(
          (GLADloadproc)glfwGetProcAddress, gl3Caps.glversion))) {
    RWERROR((ERR_GENERAL, "gladLoadGLLoader failed"));
...
static int initOpenGL(void) {
  ...
  */
  ...
  u_lightPosition =
      registerUniform("u_lightPosition", UNIFORM_VEC4, MAX_LIGHTS);
  u_lightDirection =
      registerUniform("u_lightDirection", UNIFORM_VEC4, MAX_LIGHTS);
...
static int finalizeOpenGL(void) { return 1; }
...
static int deviceSystemSDL2(DeviceReq req, void *arg, int32 n) {
  ...
  case DEVICEGETSUBSSYSTEMINFO: {
    ...
    if (display_name == nil)
      return 0;
    strncpy(((SubSystemInfo *)arg)->name, display_name,
...
static int deviceSystemGLFW(DeviceReq req, void *arg, int32 n) {
  ...
  case DEVICEGETSUBSSYSTEMINFO:
    ...
    if (n >= glGlobals.numMonitors)
      return 0;
    strncpy(((SubSystemInfo *)arg)->name, glfwGetMonitorName(monitors[n]),
            sizeof(SubSystemInfo::name));
    return 1;
